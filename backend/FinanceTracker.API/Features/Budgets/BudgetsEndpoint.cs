...
public static class BudgetsEndpoint
{
    public static void Map(WebApplication app)
    {
        ...
        group.MapGet("/", async (ClaimsPrincipal user, IMediator mediator, int? month, int? year) =>
        {
            var now = DateTime.UtcNow;
            var result = await mediator.Send(new GetBudgetsQuery(
                user.GetUserId(),
                month ?? now.Month,
                year ?? now.Year));
        ...
        group.MapDelete("/{id:guid}", async (Guid id, ClaimsPrincipal user, AppDbContext db) =>
        {
            var budget = await db.Budgets
                .FirstOrDefaultAsync(b => b.Id == id && b.UserId == user.GetUserId());
